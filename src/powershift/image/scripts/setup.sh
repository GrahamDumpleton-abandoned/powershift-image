...
if [ x"$S2I_SOURCE_PATH" != x`pwd` ]; then
    echo "ERROR: Working directory of 'setup' script is not $S2I_SOURCE_PATH."
    exit 1
fi
...
# Run any user supplied script to setup application data.

if [ -f $S2I_SOURCE_PATH/.s2i/action_hooks/setup ]; then
    if [ ! -x $S2I_SOURCE_PATH/.s2i/action_hooks/setup ]; then
        echo "ERROR: Script $S2I_SOURCE_PATH/.s2i/action_hooks/setup not executable."
        exit 1
    fi
fi

if [ -x $S2I_SOURCE_PATH/.s2i/action_hooks/setup ]; then
    echo " -----> Running $S2I_SOURCE_PATH/action_hooks/setup"
    $S2I_SOURCE_PATH/.s2i/action_hooks/setup
fi
