...
if [ x"$S2I_SOURCE_PATH" != x`pwd` ]; then
    echo "ERROR: Working directory of 'alive' script is not $S2I_SOURCE_PATH."
    exit 1
fi
...
# Run any user supplied script to test whether the application is alive.

if [ -f $S2I_SOURCE_PATH/.s2i/action_hooks/alive ]; then
    if [ ! -x $S2I_SOURCE_PATH/.s2i/action_hooks/alive ]; then
        echo "ERROR: Script $S2I_SOURCE_PATH/.s2i/action_hooks/alive not executable."
        exit 1
    fi
fi

if [ -x $S2I_SOURCE_PATH/.s2i/action_hooks/alive ]; then
    $S2I_SOURCE_PATH/.s2i/action_hooks/alive
fi
