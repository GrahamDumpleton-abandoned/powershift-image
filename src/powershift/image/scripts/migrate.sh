...
if [ x"$S2I_SOURCE_PATH" != x`pwd` ]; then
    echo "ERROR: Working directory of 'migrate' script is not $S2I_SOURCE_PATH."
    exit 1
fi
...
# Run any user supplied script to migrate any application data.

if [ -f $S2I_SOURCE_PATH/.s2i/action_hooks/migrate ]; then
    if [ ! -x $S2I_SOURCE_PATH/.s2i/action_hooks/migrate ]; then
        echo "ERROR: Script $S2I_SOURCE_PATH/.s2i/action_hooks/migrate not executable."
        exit 1
    fi
fi

if [ -x $S2I_SOURCE_PATH/.s2i/action_hooks/migrate ]; then
    echo " -----> Running $S2I_SOURCE_PATH/action_hooks/migrate"
    $S2I_SOURCE_PATH/.s2i/action_hooks/migrate
fi
